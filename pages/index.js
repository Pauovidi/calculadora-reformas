...
export default function Home(){
  ...
  const [input, setInput] = useState({
    ...
    falsoTecho_m2:0,  falsoTechoTec_m2:0,
    numHabitaciones:0,
  ...
  const setRefCiudad = ()=> update({ precioBaseM2: params.referenciaCiudad[input.ciudad] ?? 0 });
  ...
  return (
    <>
      ...
      <div className="grid lg:grid-cols-[1fr_380px] gap-6">
              ...
              <div className="grid md:grid-cols-2 gap-3">
                <div>
                  ...
                  <select className="select" value={input.tipoObra} onChange={e=>update({tipoObra: e.target.value})}>
                    <option>Reforma</option><option>Obra nueva</option>
                  </select>
                </div>
                <div>
                  <label className="label">Precio derribo (si aplica)</label>
                  <input className="input" type="number" value={input.derribo_importe}
              ...
              <div className="grid md:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input className="input flex-1" type="number" value={input.precioBaseM2}
                  ...
                  <p className="muted mt-1">
                    Ciudad: {input.ciudad} · {params.referenciaCiudad[input.ciudad] ?? 0} €/m²
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {[
                ...
                <div key={k} className="space-y-2">
                  ...
                  <input className="input" placeholder="€/m² aplicación" type="number"
                    value={input[`alic_${k}_apl`]} onChange={e=>update({[`alic_${k}_apl`]: parseFloat(e.target.value)||0})}/>
                  <input className="input" placeholder="€/m² material" type="number"
                    value={input[`alic_${k}_mat`]} onChange={e=>update({[`alic_${k}_mat`]: parseFloat(e.target.value)||0})}/>
                  <input className="input" placeholder="m² a alicatar" type="number"
                    value={input[`alic_${k}_m2`]}  onChange={e=>update({[`alic_${k}_m2`]:  parseFloat(e.target.value)||0})}/>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="card p-4 md:p-5">
            ...
            <div className="grid md:grid-cols-2 gap-3">
              <div>
                ...
                <input className="input" type="number" value={input.falsoTecho_m2}
                  onChange={e=>update({falsoTecho_m2: parseFloat(e.target.value)||0})}/>
                <p className="muted mt-1">{params.precios.falsoTecho_m2} €/m² (editable)</p>
              </div>
              <div>
                <label className="label">m² falso techo técnico</label>
                <input className="input" type="number" value={input.falsoTechoTec_m2}
                  onChange={e=>update({falsoTechoTec_m2: parseFloat(e.target.value)||0})}/>
                <p className="muted mt-1">{params.precios.falsoTechoTecnico_m2} €/m² (editable)</p>
              </div>
            </div>
          </div>
          ...
          <div className="card p-4 md:p-5">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              <div>
                ...
                <input className="input" type="number" value={input.numHabitaciones}
                  onChange={e=>update({numHabitaciones: parseInt(e.target.value)||0})}/>
                <p className="muted mt-1">{params.precios.habitacion_fija} € / habitación</p>
              </div>

              <div>
                <label className="label">Tabiquería interior</label>
                <select className="select" value={input.tabiqueria_tipo}
                  onChange={e=>update({tabiqueria_tipo: e.target.value})}>
                  <option>Ninguna</option><option>Obra</option><option>Pladur</option>
                </select>
                <input className="input mt-2" type="number" placeholder="m²"
                  value={input.tabiqueria_m2}
                  onChange={e=>update({tabiqueria_m2: parseFloat(e.target.value)||0})}/>
                <p className="muted mt-1">
                  Obra {params.precios.tabiqueria_obra_m2} €/m², Pladur {params.precios.tabiqueria_pladur_m2} €/m²
                </p>
              </div>

              <div>
                <label className="label">Baños (informativo)</label>
                <input className="input" type="number" value={input.numBanos_info}
                  onChange={e=>update({numBanos_info: parseInt(e.target.value)||0})}/>
              </div>

              <div>
                <label className="label">Reforma de cocina</label>
                <select className="select" value={input.cocina ? "Sí":"No"}
              ...
              <div>
                ...
                <select className="select" value={input.incluirFontaneria ? "Sí":"No"}
              ...
              <div>
                ...
                <select className="select" value={input.incluirElectricidad ? "Sí":"No"}
              ...
              <div>
                ...
                <select className="select" value={input.calidad}
                  onChange={e=>update({calidad: e.target.value})}>
                  {Object.keys(params.factorCalidad).map(k=>(<option key={k}>{k}</option>))}
                </select>
              </div>

              <div>
                <label className="label">Ciudad</label>
                <select className="select" value={input.ciudad}
                  onChange={e=>update({ciudad: e.target.value})}>
                  {Object.keys(params.referenciaCiudad).map(c=>(<option key={c}>{c}</option>))}
                </select>
              </div>
            </div>
          </div>
        ...
        {/* Columna derecha: precio + desglose + parámetros (acordeón oscuro) */}
        <aside className="space-y-6">
          ...
          {/* Acordeón de parámetros en sidebar (fondo gris más oscuro) */}
          <details className="rounded-2xl bg-gray-100 border">
            <summary className="px-4 py-3 cursor-pointer select-none font-semibold">
              Parámetros
