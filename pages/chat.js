  ...
  return (
    <div className="space-y-3">
      <div>
        ...
        <select className="select" value={local.ciudadPorDefecto}
          onChange={(e) => update("ciudadPorDefecto", e.target.value)}>
          {Object.keys(local.referenciaCiudad).map(c => <option key={c} value={c}>{c}</option>)}
        </select>
      </div>
        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-2">
          {Object.entries(local.referenciaCiudad).map(([city,val])=>(
            <div key={city} className="flex items-center gap-2">
              ...
              <input className="input w-24" type="number" value={val}
                onChange={e=>update(`referenciaCiudad.${city}`, parseFloat(e.target.value)||0)} />
            </div>
          ))}
        </div>
      </div>

      <button className="btn w-full" onClick={()=>{ setLocal(defaultParams); setParams?.(defaultParams); }}>
